...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # 定义不同任务的问题模板
        # 定义不同任务的问题模板
        QUESTION_TEMPLATE_JUDGE = (
            "{Question} First output the thinking process in <think> </think> tags and then output the final answer in <answer> </answer> tags. Ensure your final answer is exactly 'yes' or 'no'.")
        ...
        def make_conversation_image(example,task):
            ...
            prompt=QUESTION_TEMPLATE_JUDGE.format(Question=example["problem"])
...
def iou_reward(completions, solution, **kwargs):
    ...
    def extract_yes_no_answers(content):
        """
        比较判断题答案
        不区分大小写，只匹配独立的yes/no词
        """
        # 提取和标准化答案
        answer=None
        try:
            ...
            # 定义标准答案形式
            yes_pattern = r'(?<![a-z])(yes|true|correct)(?![a-z])'
            no_pattern = r'(?<![a-z])(no|false|incorrect)(?![a-z])'
            
            # 标准化学生答案
            if re.search(yes_pattern, content, re.IGNORECASE):
                answer = 'yes'
            elif re.search(no_pattern, content, re.IGNORECASE):
                answer = 'no'
        except:
            print(f'content is Nonetype {content}')
            pass  
        return answer
    ...
    for content, sol, task,_problem in zip(contents, solution,task_list,problem_list):
        reward = 0.0
        try:
            student_answer= extract_yes_no_answers(extract_answer_content(content))
            solution= extract_yes_no_answers(extract_answer_content(sol))
            
            if student_answer !=None:
                result = (student_answer== solution)
                print(f"Student answer: {student_answer}")
                print(f"Solution: {solution}")
                print('result',result)
                reward = 1.0 if (student_answer== solution) else 0.0
            else:
                print(f'None yes or no in answers! content:{content}')

            if task=='math': #给一次匹配机会
                answer = parse(content)
                if float(verify(answer, parse(sol))) > 0:
                    print('parse sucesuss!!!')
                    reward = 1.0
