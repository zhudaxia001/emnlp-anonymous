        ...
        if task=='detection':
            ...
            try:
                ...
                if float(verify(answer, parse(sol))) > 0:
                    ...
                    print('parse result:',f'answer:{answer}',f'solution:{parse(sol)}')
            except Exception:
                print('parse failed...')
                pass  # Continue to next verification method if this fails
            ...
            if reward == 0.0:
                try:   
                    if get_final_answer(content)==get_final_answer(sol): # 做个选择题匹配
                        reward = 1.0
                        if get_final_answer(content)!=None:
                            print('choice sucesuss!!!')
                            print('math choice:  ','student_answer',get_final_answer(content),'ground_truth',get_final_answer(sol))
                    # 如果 答案出现在回答中也算对 
                    # Extract answer from solution if it has think/answer tags
                    ground_truth = extract_answer_content(sol)
                    student_answer = extract_answer_content(content)
