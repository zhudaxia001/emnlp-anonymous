...
#             # )
#             print("The attention layers in this model are transitioning from computing the RoPE embeddings internally "
#                 "through `rotary_pos_emb` (2D tensor of RoPE theta values), to using externally computed "
#                 "`position_embeddings` (Tuple of tensors, containing cos and sin). In v4.54 `rotary_pos_emb` will be "
#                 "removed and `position_embeddings` will be mandatory."
#             )
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # 长度为 num_generations的list
        # problem: Please provide the bounding box coordinate of the region this sentence describes: pink shirt thats yellow.
        # solution: [322.86, 381.07, 454.32, 640.0]
        # prompt: [{'role': 'user', 'content': [{'type': 'image'}, {'type': 'text', 'text': 'Please provide the bounding box coordinate of the region this sentence describes: pink shirt thats yellow. First output the thinking process in <think> </think> tags and then output the final answer in <answer> </answer> tags. Output the final answer in JSON format.'}]}]
        return {
            'image': image,
...
def iou_reward(completions, solution, **kwargs):
    ...
    #print('kwargs dictionary:', kwargs_dict)
    # # 打印所有的键
    # print("\nKeys in kwargs:")
    ...
    for content, sol, task in zip(contents, solution,task_list):
        reward = 0.0
        # Try symbolic verification first
        if task=='detection':  
            # Detection iou_reward
            try:
