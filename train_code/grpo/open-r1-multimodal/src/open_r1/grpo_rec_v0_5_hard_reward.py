...
def iou_reward(completions, solution, **kwargs):
    ...
    def process_answer(content,task_type):
        ...
        if full_match := re.search(r'<answer>(.*?)</answer>', content, re.DOTALL):
            ...
            if task_type!='classify' and task_type!='classify':
                return answer
    ...
    def calculate_list_hard_reward(pred_list, target_list):
        """
        计算两个列表的 reward
        ...
        # 判断 reward
        if not intersection:  # 没有交集
            return 0.0
        elif pred_set == target_set:  # 完全相同
            return 1.0
        else:  # 有交集但不完全相同
            return 0.2
    ...
    for content, sol, task,_problem in zip(contents, solution,task_list,problem_list):
        ...
        if task=='detection':
            ...
            try:
                ...
                reward = calculate_list_hard_reward(ground_truth_list,student_answer_list)
                print('reward',reward)
            except Exception:
